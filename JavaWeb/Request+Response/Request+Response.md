...
**Request是请求对象，Response是响应对象**。这两个对象在使用Servlet的时候有看到：![1628735216156](assets/1628735216156.png)

此时，需要思考一个问题request和response这两个参数的作用是什么？
...
目前只知道这两个对象是用来干什么的，那么它们具体是如何实现的，需要继续深入学习。接下来，就先从Request对象来学习，主要学习下面这些内容：
...
在学习这节内容之前，先思考一个问题，前面在介绍Request和Reponse对象的时候，发现：
...
HTTP请求数据总共分为三部分内容，分别是**请求行、请求头、请求体**。对于这三部分内容的数据，分别该如何获取，首先学习请求行数据如何获取？
...
浏览器在发送**GET请求的时候是没有请求体**的，所以需要把请求方式变更为POST，请求体中的数据格式如下：
...
>具体实现的步骤如下：
>
>1.准备一个html页面，在页面中添加form表单，用来发送post请求
>
...
3. 调用**getReader()**或者**getInputStream()**方法，因为目前前端传递的是**纯文本数据**，所以采用**getReader()方法**来获取
...
* 请求行
  ...
  * getQueryString()获取GET请求方式的**请求参数**
* 请求头
...
5.**GET请求和POST请求获取请求参数的方式不一样，在获取请求参数这块该如何实现呢**？
...
(2)浏览器在发送HTTP的过程中会对中文数据进行**URL编码**
...
但是对于上面的计算过程，如果没有工具，纯手工计算的话，相对来说还是比较复杂的，也不需要进行手动计算，在Java中已经提供了编码和解码的API工具类可以让我们更快速的进行编码和解码：
...
* 在进行编码和解码的时候，不管使用的是哪个字符集，对应的`%E5%BC%A0%E4%B8%89`是一致的
...
* 所以可以考虑把`å¼ ä¸`转换成字节，在把字节转换成`张三`，在转换的过程中是它们的编码一致，就可以解决中文乱码问题。
...
介绍完这些方法后，通过案例把这些方法都用一用，首先先来完成下重定向的功能开发。
...
(2)资源A现在无法处理该请求，就会给浏览器响应一个302的 **状态码+location** 的一个访问资源B的路径
...
虽然功能已经实现，但是从设置重定向的两行代码来看，会发现除了重定向的地址不一样，其他的内容都是一模一样，所以request对象提供了简化的编写方式为：
...
介绍完**请求重定向和请求转发**以后，接下来需要把这两个放在一块对比下：
...
答案也比较简单，可以在代码中动态去获取项目访问的虚拟目录，具体如何获取，可以借助前面所学习的request对象中的getContextPath()方法，修改后的代码如下：
...
上述代码中，对于流的copy的代码还是比较复杂的，所以建议使用别人提供好的方法来简化代码的开发，具体的步骤是：
...
接下来通过两个比较常见的案例，一个是**注册**，一个是**登录**来对上面的内容进行一个实战演练，首先来实现用户登录。
